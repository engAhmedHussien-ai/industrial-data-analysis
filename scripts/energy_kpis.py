...
# Plotting
# =========================
def save_energy_plots(df: pd.DataFrame, output_dir: Path):
    output_dir.mkdir(parents=True, exist_ok=True)

    # Power trend
    plt.figure(figsize=(12, 4))
    plt.plot(df.index, df["power_kW"], label="Power (kW)")
    plt.title("Power Consumption Over Time")
    plt.ylabel("kW")
    plt.tight_layout()
    plt.savefig(output_dir / "power_trend.png")
    plt.close()

    # Load distribution
    plt.figure(figsize=(6, 4))
    df["load_pct"].hist(bins=30)
    plt.title("Load Distribution (%)")
    plt.xlabel("Load %")
    plt.tight_layout()
    plt.savefig(output_dir / "load_distribution.png")
    plt.close()

    print("Energy plots saved.")
...
def main():
    ...
    print("Saving energy plots...")
    save_energy_plots(df, Path("reports"))
